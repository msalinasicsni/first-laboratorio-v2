...
public class RecepcionMxController {
                            ...
                            for (DaSolicitudDx solicitudDx : solicitudDxList) {
                                if (!solicitudDx.getAprobada() && solicitudDx.getCodDx().getNombre().toLowerCase().contains("ifi virus respiratorio")) {
                                    ...
                                    for (OrdenExamen examen : ordenesExamen) {
                                        //no tiene resultado registrado aÃºn
    ...
    /***
     * Método para registrar resultado de una orden examen como resultado final de la solicitud a la que pertenece dicha orden
    ...
    private String RecepcionMxToJson(List<RecepcionMx> recepcionMxList) throws Exception{
        ...
        for(RecepcionMx recepcion : recepcionMxList){
            ...
            if (mostrar) {
                ...
                if (!recepcion.getTomaMx().getIdNotificacion().getCodTipoNotificacion().equalsIgnoreCase("TPNOTI|VIH")) {
                    if (fechaInicioSintomas != null) {
                        map.put("fechaInicioSintomas", DateUtil.DateToString(fechaInicioSintomas, "dd/MM/yyyy"));
                    ...
                    map.put("fechaInicioSintomas", " ");
                    //para VIH los dias son de la fecha de toma hasta la fecha actual en que se estÃ¡ recepcionando en lab
                    ...
                    if (tomaMx != null) {
                        ...
                        for (Area area : areaDxList) {
                            if (esRecepcionista != null || seguridadService.usuarioAutorizadoArea(userName, area.getIdArea())) {
                                ...
                                String apellidos = "";
                                String edad = "";
                                if (!tomaMx.getIdNotificacion().getCodTipoNotificacion().equalsIgnoreCase("TPNOTI|VIH")) {
                                    if (tomaMx.getIdNotificacion().getPersona() != null) {
                                ...
                                y = y - 4;
                                DatosRecepcionMx recepcionMx = recepcionMxService.getRecepcionMxByCodUnicoMxV2(tomaMx.getCodigoUnicoMx(), labProcesa.getCodigo());
                                String procesadoPor = "";
                                String aprobadoPor = "";
